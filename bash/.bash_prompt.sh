...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "warn" slices
  __promptline_wrapper "$(if [ "$(git status --porcelain 2> /dev/null | wc -l)" -gt "0" ];then echo "$bldred*";fi)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline_right_prompt {
  ...
  # section "warn" slices
  __promptline_wrapper "$(if [ "$(git status --porcelain 2> /dev/null | wc -l)" -gt "0" ];then echo "$bldred*";fi)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
